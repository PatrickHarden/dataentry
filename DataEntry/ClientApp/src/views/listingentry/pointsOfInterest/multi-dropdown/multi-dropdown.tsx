...
import { PointOfInterestPlace } from '../../../../types/listing/pointsOfInterest';
...
const MultiDropdown: FC<any> = (props) => {
    ...
    const { kind, travelMode, distanceUnits, deleteRow, index, prefix, values, name, changeHandler, interestIndex } = props;
    ...
    return (
        <GLForm initVals={values}
            ...
            <MultiDropdownContainer data-testid="points-of-interest-row" index={index}>
                <Row key={Date.now()}>
                    <Col xs={2.5}>
                        <GLField<FormInputProps> use={FormInput} defaultValue={values.name} name="name" label={index === 0 ? 'Name' : undefined} />
                    </Col>
                    <Col xs={2.5}>
                        <GLField<FormInputProps> use={FormInput} defaultValue={values.type} disabled={false} name="type" label={index === 0 ? 'Type' : undefined} />
                    </Col>
                    <Col xs={1.5}>
