...
{
    public class DataEntryMutationPublishListingTest
    {
        private readonly FieldType _listingField;

        public DataEntryMutationPublishListingTest()
        {
            // Mock service layer
            var publishingService = new Mock<IPublishingService>();
            publishingService.Setup(x => x.PublishListingAsync(It.IsAny<int>(), It.IsAny<ClaimsPrincipal>()))
            ...
            _listingField = mutation.GetField("publishListing");
        }
